...
@router.get("/subscriptions/expiring-users")
async def get_expiring_users(
    ...
    """获取即将过期的订阅用户列表"""
    try:
        expiring_users = subscription_service.check_subscription_expiry(db)
        return {
            "expiring_users": expiring_users,
            "total_count": len(expiring_users),
            "message": f"Found {len(expiring_users)} users with subscriptions expiring within 7 days"
        }
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get expiring users"
        )
...
async def process_expired_subscriptions(
    ...
    """处理过期订阅（将过期用户降级到免费版）"""
    try:
        result = subscription_service.process_expired_subscriptions(db)
        return result
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to process expired subscriptions"
        )


@router.get("/subscriptions/analytics-detailed")
async def get_detailed_subscription_analytics(
    admin_user: User = Depends(get_admin_user),
    db: Session = Depends(get_db)
):
    """获取详细的订阅分析数据"""
    try:
        analytics = subscription_service.get_subscription_analytics(db)
        return analytics
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def admin_upgrade_user_subscription(
    user_id: int,
    new_tier: str,
    reason: Optional[str] = None,
    admin_user: User = Depends(get_admin_user),
    ...
    try:
        ...
        result = subscription_service.upgrade_subscription(
            ...
            new_tier=tier,
            db=db,
            reason=f"Admin upgrade by {admin_user.username}: {reason}" if reason else f"Admin upgrade by {admin_user.username}"
        )
...
@router.post("/users/{user_id}/subscription/downgrade")
async def admin_downgrade_user_subscription(
    user_id: int,
    new_tier: str,
    reason: Optional[str] = None,
    admin_user: User = Depends(get_admin_user),
    ...
    try:
        ...
        result = subscription_service.downgrade_subscription(
            ...
            new_tier=tier,
            db=db,
            reason=f"Admin downgrade by {admin_user.username}: {reason}" if reason else f"Admin downgrade by {admin_user.username}"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to downgrade user subscription"
        )
...
async def admin_cancel_user_subscription(
    ...
    try:
        result = subscription_service.cancel_subscription(
            user=user,
            db=db,
            reason=f"Admin cancellation by {admin_user.username}: {reason}" if reason else f"Admin cancellation by {admin_user.username}",
            immediate=immediate
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to cancel user subscription"
        )
...
async def get_user_subscription_history(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get user subscription history"
        )
...
@router.get("/security/rate-limits/{identifier}")
async def get_user_rate_limit_status(
    ...
    """获取特定用户/IP的速率限制状态"""
    try:
        status_info = rate_limiter.get_user_status(identifier)
        return status_info
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get user rate limit status: {str(e)}"
        )

@router.post("/security/rate-limits/{identifier}/reset")
async def reset_user_rate_limits(
    ...
    """重置用户/IP的速率限制"""
    try:
        rate_limiter.reset_user_limits(identifier)
...
async def get_all_sessions(
    ...
    """获取所有活跃会话"""
    try:
        stats = session_service.get_global_session_stats()
        return stats
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_login_attempts(
    ...
    """获取登录尝试统计"""
    try:
        stats = login_limiter.get_global_stats()
        return stats
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get login attempt stats: {str(e)}"
        )

@router.get("/security/login-attempts/{identifier}")
async def get_user_login_attempts(
    ...
    """获取特定用户/IP的登录尝试信息"""
    try:
        info = login_limiter.get_attempt_info(identifier)
        return info
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get login attempt info: {str(e)}"
        )

@router.post("/security/login-attempts/{identifier}/reset")
async def reset_login_attempts(
    ...
    """重置用户/IP的登录尝试记录"""
    try:
        login_limiter.clear_attempts(identifier)
        return {"message": f"Login attempts reset for {identifier}"}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def add_to_blacklist(
    ...
    """添加IP到黑名单"""
    try:
        security_middleware.add_to_blacklist(request.ip)
        logger.info(f"Admin {admin_user.email} added {request.ip} to blacklist. Reason: {request.reason}")
        return {"message": f"IP {request.ip} added to blacklist"}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to add IP to blacklist: {str(e)}"
        )
...
async def remove_from_blacklist(
    ...
    try:
        ...
        logger.info(f"Admin {admin_user.email} removed {ip} from blacklist")
...
async def add_to_whitelist(
    ...
    """添加IP到白名单"""
    try:
        security_middleware.add_to_whitelist(request.ip)
        logger.info(f"Admin {admin_user.email} added {request.ip} to whitelist. Reason: {request.reason}")
...
async def get_security_stats(
    ...
    try:
        stats = {
            "rate_limits": rate_limiter.get_global_stats(),
            "sessions": session_service.get_global_session_stats(),
            "login_attempts": login_limiter.get_global_stats(),
