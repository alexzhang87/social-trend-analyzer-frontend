...
def get_database_models():
    ...
    return get_db, User, CreditTransaction, SubscriptionTier
...
# 定义产品和价格ID映射
# 这些价格ID由setup_stripe_products.py脚本自动创建
STRIPE_PRICE_IDS = {
    # 订阅计划价格ID（月付）
    "starter_monthly": "price_1SCXv8LSYcMrFhJ2ZL5hJDm3",  # Starter计划 $9.99/月
    "pro_monthly": "price_1SCXv9LSYcMrFhJ2iYdoc9V4",      # Pro计划 $29.99/月
    "plus_monthly": "price_1SCXvALSYcMrFhJ2nN15UUTJ",     # Plus计划 $59.99/月
    "enterprise_monthly": "price_1SCXvALSYcMrFhJ2nYDD7qor", # Enterprise计划 $199.99/月
    
    # 积分包价格ID（一次性购买）
    "credits_small": "price_1SCXvBLSYcMrFhJ2w6VQEzhp",   # 小积分包 100积分 $9.99
    "credits_medium": "price_1SCXvBLSYcMrFhJ2G2YYw6dj",  # 中积分包 500积分 $39.99
    "credits_large": "price_1SCXvCLSYcMrFhJ2Ahz6ac8r"    # 大积分包 1000积分 $69.99
}
...
async def create_checkout_session(
    ...
    db: Session = Depends(lambda: get_database_models()[0]()),
    ...
    try:
        if product_type == "subscription":
            # 订阅产品
            if product_id not in ["starter", "pro", "plus", "enterprise"]:
...
async def stripe_webhook(
    ...
    """处理Stripe Webhook事件"""
    try:
        get_db, User, CreditTransaction, SubscriptionTier = get_database_models()
...
async def handle_subscription_payment(user, product_id: str, db: Session):
    """处理订阅支付"""
    try:
        get_db, User, CreditTransaction, SubscriptionTier = get_database_models()
        ...
        if new_tier == SubscriptionTier.STARTER:
            ...
            monthly_credits = 60
        elif new_tier == SubscriptionTier.PLUS:
            monthly_credits = 150
...
async def handle_credits_payment(user, product_id: str, db: Session):
    """处理积分包支付"""
    try:
        get_db, User, CreditTransaction, SubscriptionTier = get_database_models()
