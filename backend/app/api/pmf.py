...
class PMFAnalysisResponse(BaseModel):
    id: str
    product_name: str
    pmf_score: int
...
@router.get("/analysis", response_model=PMFAnalysisResponse)
async def get_pmf_analysis(
    current_user = Depends(get_current_active_user),
    db = Depends(lambda: get_database_models()[0]())
):
    """获取PMF分析结果"""
    try:
        # 获取最新的PMF验证记录
        user_validations = [
            validation for validation in pmf_storage["validations"].values()
            if validation.get("user_id") == current_user.id
        ]
        
        if not user_validations:
            raise HTTPException(status_code=404, detail="未找到PMF验证记录")
        
        # 获取最新的验证记录
        latest_validation = max(user_validations, key=lambda x: x["created_at"])
        
        return PMFAnalysisResponse(
            id=latest_validation["id"],
            product_name=latest_validation["product_name"],
            pmf_score=latest_validation["pmf_score"],
            analysis_results=latest_validation["validation_results"],
        ...
        logger.error(f"获取PMF分析失败: {e}")
        raise HTTPException(status_code=500, detail=f"获取PMF分析失败: {str(e)}")
