...
from .api import trends, health, seed, reports, debug, auth, admin, credits, feedback, google_trends, monkeylearn, data_studio, metabase, payments, cache, cache_optimization, monitoring, documentation, analysis, pmf, founder, ai_expert, ai_insights, enhanced_consultant
from .api.v1 import websocket, optimization, mobile, chat, feedback as v1_feedback, data_quality, ai_analysis
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # 启动积分过期清理定时任务
        try:
            from .tasks.credit_expiry_task import start_credit_expiry_scheduler
            import asyncio
            
            # 创建后台任务
            asyncio.create_task(start_credit_expiry_scheduler())
            logger.info("INFO:     Credit expiry scheduler started")
        except Exception as e:
            logger.error(f"ERROR:     Failed to start credit expiry scheduler: {e}")
...
app.include_router(ai_expert.router, prefix=f"{api_prefix}/ai-expert", tags=["ai-expert"])  # 添加AI专家路由
app.include_router(ai_insights.router, prefix=f"{api_prefix}/ai-insights", tags=["ai-insights"])  # 添加AI洞察路由
app.include_router(enhanced_consultant.router, prefix=f"{api_prefix}/enhanced-consultant", tags=["enhanced-consultant"])  # 添加增强型创业顾问路由
app.include_router(chat.router, prefix=f"{api_prefix}/chat", tags=["chat"])  # 添加聊天API路由
app.include_router(v1_feedback.router, prefix=f"{api_prefix}/feedback", tags=["feedback-v1"])  # 添加v1反馈API路由
app.include_router(data_quality.router, prefix=f"{api_prefix}/data-quality", tags=["data-quality"])  # 添加数据质量API路由
app.include_router(ai_analysis.router, prefix=f"{api_prefix}/ai-analysis", tags=["ai-analysis"])  # 添加AI分析API路由
