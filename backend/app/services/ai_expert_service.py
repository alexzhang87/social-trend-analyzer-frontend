...
from .enhanced_startup_consultant_service import EnhancedStartupConsultantService

class AIExpertService:
    def __init__(self):
        self.llm_provider = get_llm_provider()
        self.enhanced_consultant = EnhancedStartupConsultantService()
        
        # Expert types and their specializations
        self.expert_types = {
            "enhanced_startup_consultant": {
                "name": "Enhanced AI Startup Consultant",
                "description": "AI-powered startup consultant trained on comprehensive business data",
                "expertise": ["comprehensive_analysis", "ai_insights", "data_driven_advice", "startup_optimization"],
                "prompt_template": self._get_enhanced_consultant_prompt(),
                "uses_enhanced_model": True
            },
            "business_strategist": {
    ...
    async def generate_expert_response(self, session_id: str, user_message: str, 
        ...
        try:
            ...
            # Check if this expert uses the enhanced model
            if expert_info.get("uses_enhanced_model", False):
                # Use enhanced startup consultant model
                enhanced_advice = self.enhanced_consultant.generate_startup_advice(
                    idea_description=session_data["context_data"]["original_idea"],
                    ...
                    target_market=context.get("target_market") if context else None
                )
                
                # Format the enhanced advice as a conversational response
                expert_response = self._format_enhanced_advice_as_response(enhanced_advice, user_message)
                follow_up_questions = self.enhanced_consultant.generate_follow_up_questions(enhanced_advice)
            else:
                # Use traditional LLM approach
    ...
    def _get_enhanced_consultant_prompt(self) -> str:
        return """
        You are an Enhanced AI Startup Consultant powered by advanced machine learning models trained on comprehensive business data. Your expertise includes:
        - Comprehensive startup analysis using AI-driven insights
        - Data-driven business model recommendations
        - Market opportunity assessment with predictive analytics
        - Technical roadmap planning with industry best practices
        - Risk assessment and mitigation strategies
        - Success metrics and KPI optimization

        User's business idea: {idea}
        PMF Data: {pmf_data}
        Analysis Data: {analysis_data}

        Provide comprehensive, data-driven advice that leverages AI insights to help startups succeed. Focus on actionable recommendations backed by data analysis and industry patterns.
        """
    
    def _format_enhanced_advice_as_response(self, enhanced_advice: Dict[str, Any], user_message: str) -> str:
        """Format enhanced advice as a conversational response"""
        try:
            advice = enhanced_advice.get("advice", {})
            
            # Create a conversational response based on the user's question
            response_parts = []
            
            # Start with acknowledgment
            response_parts.append(f"基于我们的AI增强分析，我来为您解答关于「{user_message}」的问题。")
            
            # Add executive summary
            if advice.get("executive_summary"):
                response_parts.append(f"\n**整体分析：**\n{advice['executive_summary']}")
            
            # Add relevant sections based on user question keywords
            user_msg_lower = user_message.lower()
            
            if any(keyword in user_msg_lower for keyword in ["市场", "竞争", "机会"]):
                market_analysis = advice.get("market_analysis", {})
                if market_analysis:
                    response_parts.append(f"\n**市场分析：**")
                    response_parts.append(f"• 市场规模：{market_analysis.get('market_size', '需要进一步分析')}")
                    response_parts.append(f"• 竞争程度：{market_analysis.get('competition_level', '中等')}")
                    if market_analysis.get("opportunities"):
                        response_parts.append(f"• 主要机会：{', '.join(market_analysis['opportunities'][:3])}")
            
            if any(keyword in user_msg_lower for keyword in ["商业模式", "收入", "盈利"]):
                business_model = advice.get("business_model_recommendations", {})
                if business_model:
                    response_parts.append(f"\n**商业模式建议：**")
                    if business_model.get("revenue_streams"):
                        response_parts.append(f"• 收入来源：{', '.join(business_model['revenue_streams'][:3])}")
                    cost_structure = business_model.get("cost_structure", {})
                    if cost_structure.get("optimization_tips"):
                        response_parts.append(f"• 成本优化：{cost_structure['optimization_tips']}")
            
            if any(keyword in user_msg_lower for keyword in ["技术", "开发", "实现"]):
                tech_roadmap = advice.get("technical_roadmap", {})
                if tech_roadmap:
                    response_parts.append(f"\n**技术路线图：**")
                    if tech_roadmap.get("mvp_features"):
                        response_parts.append(f"• MVP功能：{', '.join(tech_roadmap['mvp_features'][:3])}")
                    if tech_roadmap.get("development_phases"):
                        first_phase = tech_roadmap['development_phases'][0]
                        response_parts.append(f"• 首个阶段：{first_phase.get('phase')} ({first_phase.get('duration')})")
            
            if any(keyword in user_msg_lower for keyword in ["风险", "挑战", "问题"]):
                risk_assessment = advice.get("risk_assessment", {})
                if risk_assessment:
                    response_parts.append(f"\n**风险评估：**")
                    if risk_assessment.get("high_risks"):
                        response_parts.append(f"• 主要风险：{', '.join(risk_assessment['high_risks'][:2])}")
                    if risk_assessment.get("mitigation_strategies"):
                        response_parts.append(f"• 应对策略：{', '.join(risk_assessment['mitigation_strategies'][:2])}")
            
            # Add next steps if no specific area was asked about
            if len(response_parts) <= 2:  # Only acknowledgment and summary
            ...
            model_info = enhanced_advice.get("model_info", {})
            confidence = model_info.get("confidence_score", 0.8)
            response_parts.append(f"\n*此分析基于我们的AI增强模型，置信度：{confidence:.0%}*")
            
            return "\n".join(response_parts)
            
        except Exception as e:
            logger.error(f"Error formatting enhanced advice: {str(e)}")
            return f"基于您的问题「{user_message}」，我正在为您分析相关的创业建议。让我为您提供一些专业的指导意见。"