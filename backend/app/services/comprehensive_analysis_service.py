...
from .hacker_news_service import HackerNewsService
from .techcrunch_rss_service import TechCrunchRSSService
...
class ComprehensiveAnalysisService:
    ...
    def __init__(self):
        ...
        self.techcrunch_service = TechCrunchRSSService()
    ...
    async def analyze_trends_comprehensive(
        ...
        if platforms is None:
            platforms = ['twitter', 'reddit', 'product_hunt', 'google_trends', 'hacker_news', 'techcrunch', 'stackoverflow']
        ...
        if 'hacker_news' in platforms:
            tasks.append(self._get_hacker_news_data(keywords, limit_per_platform))
        if 'techcrunch' in platforms:
            tasks.append(self._get_techcrunch_data(keywords, limit_per_platform))
        if 'stackoverflow' in platforms:
            tasks.append(self._get_stackoverflow_data(keywords, time_filter, limit_per_platform))
        
        # 执行所有任务
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        # 处理结果
        platform_keys = [p for p in platforms if p in ['twitter', 'reddit', 'product_hunt', 'google_trends', 'hacker_news', 'techcrunch', 'stackoverflow']]
    ...
    async def _get_hacker_news_data(self, keywords: List[str], limit: int) -> List[Dict[str, Any]]:
        """获取Hacker News数据"""
        try:
            return await self.hacker_news_service.search_stories(keywords, limit)
        except Exception as e:
            logger.error(f"获取Hacker News数据失败: {e}")
            return []
    
    async def _get_techcrunch_data(self, keywords: List[str], limit: int) -> List[Dict[str, Any]]:
        """获取TechCrunch数据"""
        try:
            query = ' '.join(keywords)
            return await self.techcrunch_service.search_articles(query, limit)
        except Exception as e:
            logger.error(f"获取TechCrunch数据失败: {e}")
            return []
    
    async def _get_stackoverflow_data(self, keywords: List[str], time_filter: str, limit: int) -> List[Dict[str, Any]]:
        """获取Stack Overflow数据"""
        try:
            query = ' '.join(keywords)
            return await self.stackoverflow_service.search_questions(query, limit, time_filter=time_filter)
        except Exception as e:
            logger.error(f"获取Stack Overflow数据失败: {e}")
            return []
