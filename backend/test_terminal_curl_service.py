...
from app.services.terminal_curl_service import TerminalCurlService
from app.utils.logger import logger

def test_terminal_curl_service():
    """
    测试完全模拟终端的 curl Twitter 服务
    """
    logger.info("=== 测试 TerminalCurlService ===")
    
    # 初始化服务
    service = TerminalCurlService()
    ...
    logger.info("\n=== 测试完成 ===")
    logger.info("🎉 所有测试通过！TerminalCurlService 工作正常")
    return True

if __name__ == "__main__":
    success = test_terminal_curl_service()
    if not success:
        logger.error("❌ 服务测试失败")
        sys.exit(1)
    else:
