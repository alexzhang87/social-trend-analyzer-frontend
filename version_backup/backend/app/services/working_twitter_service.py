...
class WorkingTwitterService:
    ...
    def search_tweets_enhanced(self, query: str, count: int = 20) -> List[Dict[str, Any]]:
        """
        增强的推文搜索，包含文本分析
        ...
        Returns:
            包含文本分析的推文列表
        """
        logger.info(f"增强推文搜索: {query}")
        
        # 先获取基础数据（通过用户搜索模拟）
        users = self.search_users(query, count)
        ...
        for user in users:
            try:
                # 构建模拟推文内容
                tweet_text = f"关于{query}的讨论: {user.get('description', '暂无描述')}"
                
                # 执行文本分析
                sentiment_analysis = enhanced_text_analysis_service.analyze_sentiment_comprehensive(tweet_text)
                keywords_extracted = enhanced_text_analysis_service.extract_keywords(tweet_text, max_keywords=8)
                text_stats = enhanced_text_analysis_service.analyze_text_statistics(tweet_text)
                ...
                tweet = {
                    ...
                    "score": user.get('followers_count', 0) // 100,  # 模拟点赞数
                    "platform_specific": {
                        "twitter_id": f"tweet_{user.get('id', '')}",
                        "user_verified": user.get('verified', False),
                        "user_followers": user.get('followers_count', 0),
                        "retweet_count": 0,
                        "favorite_count": user.get('followers_count', 0) // 100
                    },
                    "text_analysis": {
                ...
                logger.error(f"Twitter文本分析失败: {e}")
                # 即使分析失败，也保留基础数据
                tweet = {
                    "url": f"https://twitter.com/{user.get('screen_name', 'unknown')}",
                    "content": f"关于{query}的讨论: {user.get('description', '暂无描述')}",
                    "author": user.get('screen_name', 'unknown'),
                    "source": "twitter",
                    "published_at": datetime.now().isoformat(),
                    "score": user.get('followers_count', 0) // 100
                }
                enhanced_tweets.append(tweet)
        
        logger.info(f"Twitter增强搜索完成，返回 {len(enhanced_tweets)} 条分析过的推文")
        return enhanced_tweets[:count]
